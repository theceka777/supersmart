...
**Status:** v1.53 — **Round screen fully translated from Claude Design + Ghost concept full removal + closing CTA dropped** (session 32, 2026-05-16). Three workstreams in one device-verified code pass. **(1) Round screen ported from Claude Design → React Native.** 16 new components under `supersmart/components/round/` implementing Layout A "stadium": **RoundSunburst** (tier-driven rotation 60s→8.57s via `useFrameCallback`), **OutsideInPulse** (continuous sine-wave loop, `mixBlendMode: 'multiply'` matching the design's darkening intent, opacity 45% peak, tier-paced 1.4s/1.1s/0.9s/0.7s ladder, color stops 35% transparent → 65% color for a soft anchored ring), **HalftoneOverlay** (tier-driven opacity + pulse rate), **EdgeGlow** (tier-tuned curve `80% / 0.20` → `22% / 0.93`, rendered BEHIND question card + buttons not over — without RN's missing blur filter, putting it above the UI read as a tinted overlay instead of ambient stage glow), **VS strip** with `PlayerBlock` + `VsBadge` (YOU vs ECHO·47 / WORLD), **TimerPill** (pink for Quickmatch, cyan for Daily Race, low-time pulse), **StreakChip** (tier-colored, pulses at tier ≥ 2), **QuestionCard** (bordered hero with per-question 280ms fade-in), **AnswerButton** (chunky 3D with state-aware faces + hype-tier borders + X-mark SVG on wrong + per-button lightning), **FeedbackPopup** (correct/wrong/penalty kinds, 900ms bouncy entry), **Nudge** (first-speed-bonus + first-3-streak onboarding pulses, 2200ms envelope), **FullscreenLightning** (sub-2s burst, 5 SVG forks + soft radial halo, 720ms envelope), **UnstoppableBanner** (10-streak red+yellow pill, 1500ms spring entry), **RoundLayoutA** (the composition). Wired into `app/echo.tsx` (mode=quickmatch, pink) and `app/daily.tsx` (mode=dailyrace, cyan, opponent slot = `WORLD` placeholder until Phase 4 daily-leader data). State machines in echo/daily unchanged — round/* is the visual layer only. **One crash bug fixed mid-session:** React rules-of-hooks violation in 7 components (`useAnimatedStyle` was called after early-return conditionals — when the conditional flipped on first answer, hook count changed and React crashed). Fix: every conditional return moved BELOW the hook calls. **(2) Ghost concept removed end-to-end.** Session 7 (2026-04-19) decision removed "ghost" from customer-facing copy, but 5 strings missed the rename — `FINDING YOUR GHOST` matching title, `👻 GHOST WINS` loss headline, `NEW GHOST` replay button, profile notifications sub-label, challenge body. Fixed all 5. CD extended scope to all *internal* code: `GHOST_NAMES → OPPONENT_NAMES`, `ghost` state object → `opponent`, `ghostScore → opponentScore`, every style name renamed (`ghostMini → oppMini` etc), all comments scrubbed via bulk sed across 7 files. Appendix C glossary updated to match: **`Ghost run` → `Recorded run`**, **`Bot ghost` → `Bot opponent`**, `Baton pass` def stripped of ghost reference. Echo technical term retained as internal architecture name (was always positioned as the ghost alternative). Mothership decision log historical entries keep original "ghost" wording — they're audit trail. **(3) Closing CTA removed from home screen footer.** `Get super smart today by getting Super Smart today!` was voice-bearing brand DNA listed in Part 2 ("What stays from 2012") and the primer's Voice Cheat Sheet. CD removed it from the home footer. Both doc references updated to match the shipped state. Net Appendix D opens: unchanged at ~30 (no new items added; today was code work + spec realignment). v1.52 — **Launch-prep ops cluster parked (4 items)** (session 31e, 2026-05-16). CD walked through the launch-prep cluster (#38 PR strategy, #39 beta tester list, #41 App Store screenshots brief, #42 IAP sandbox testing) and parked all four as "revisit closer to launch / testing stage" — the right discipline at 12 weeks out. Park rationale per item: **#38 PR strategy** — specifics (embargo timing, exact outlet list, story-angle copy) get sharper closer to launch; warm-list relationship-warming (Twitter follows, occasional comments on coverage) can start now without locking the full spec. **#39 beta tester list** — CD already has the friend list ready; no need to formalize shape today. **#41 App Store screenshots + preview video brief** — production needs final game assets to film; brief comes alive in late Phase 6. **#42 IAP sandbox testing strategy** — revisit at testing stage, but two important context updates surfaced and are locked: **(a) WhatsApp group replaces Discord** as the beta feedback channel (CD's existing contacts already there, zero adoption friction); **(b) two-phase beta plan** documented — **Phase A: Expo Go beta** (visual / UX / gameplay feedback, can start today, no IAP testable), **Phase B: TestFlight beta** (~4–6 weeks pre-launch once Phase 4 backend + IAP wired; the full #42 sandbox testing plan applies here). Each parked item retains its open-status but gains a "parked / revisit at X" tag in Appendix D so future sessions don't re-litigate the deferral. Net: ~30 Appendix D opens unchanged (parking is not resolution; these items still need locking before launch, just not today). v1.51 — **Streak Shield pricing reversal: 3-pack removed, 1+2 pack prices raised** (session 31d, 2026-05-16). Reversal of the v1.48 lock on #27 made within the same day after CD revisited the strategic Pro-positioning. **New pricing: `$1.99` / `$3.49`** for 1 / 2 packs respectively. **3-pack removed entirely.** Strategic intent: at $3.49, the 2-pack sits $1.50 below Pro at $4.99 — the natural upsell moment where "for $1.50 more, get unlimited rounds + seasonal packs + weekly free shield + Pro avatar items + Global all-time view + Pro badge" makes Pro the obvious step up. Removing the 3-pack eliminates the structural redundancy with Pro's weekly shield grant (the 3-pack was the "stock-up" SKU; Pro is the "never think about it" SKU — both competing for the same purchase intent). Considered three execution options before locking: (A) keep 1+2 cheap and jump 3-pack to $4.99 as a Pro-decoy (strictly-dominated math, dark-pattern risk); (B) shift entire curve up to `$1.99 / $3.49 / $4.99` with 3-pack matching Pro (honest math, redundant SKU); (C) remove 3-pack entirely with new prices for remaining SKUs — chosen. The earlier v1.48 prices (`$0.99 / $1.79 / $2.49`) are formally superseded; entry preserved in #27 with strikethrough for audit trail. Two downstream UX implications: **(a)** retroactive shield repair purchase sheet (#19) now shows only 1-pack and 2-pack instead of three options — updated inline in #19; **(b)** new Phase 5/6 UX opportunity to fire a Pro upsell at the 2-pack purchase moment ("buying shields? Pro covers this and more for $1.50 more") — flagged as future follow-up but not blocking. No code changes; ASO copy + IAP product registration in App Store Connect updates accordingly when Phase 6 prep starts. v1.50 — **Streak Shield system fully specced — #17 + #19 closed** (session 31c, 2026-05-16). Closes the entire Streak Shield feature: design + pricing (#27, v1.48) + Pro grant (#17) + retroactive repair (#19) all locked. **#17 Pro weekly auto-grant** — fires Monday 6am ET (matches Daily Race / League reset anchor — one clock for the whole system); skips silently at the 3-shield cap (no banked credit, no carryover); immediate "welcome to Pro" grant on first-ever upgrade as a feel-good moment, then weekly cadence resumes; refund-and-rebuy cycles do NOT re-fire the welcome grant (tracked via `pro_entitlements.first_granted_at`); retroactive grant on next app open if a Monday was missed (cap-aware, only the most recent missed Monday gets retroactive grant — no stacking past 3); **pull-pattern implementation** via Edge Function `check_pro_shield_grant` called on app open + Pro upgrade — chosen over cron-push for ops simplicity (no cron monitoring), natural inactive-user handling (no wasted writes), and free retroactive logic. **#19 Retroactive 48h shield repair** — 48h window measured from **start-of-next-day 6am ET reset** (the moment the streak officially broke, not the start of the missed day itself) — gives players who discover the miss on next-day open a full ~48h conscious-window. Server-authoritative miss detection runs on both Daily Race submission attempt AND app open via new Edge Function `check_streak_state`. Retroactive window only opens if the player had **0 shields** at miss time — proactive auto-activate (specced 2026-04-19 session 6) takes precedence for anyone holding a shield. During the 48h window: persistent in-app banner above the Daily Race card with live countdown (`Repair your 12-day streak — 36h left`); push notifications at T+4h (~10am ET morning of next day) and T+40h (final 8h reminder); banner tap opens focused purchase sheet at standard #27 pricing. Only the **most recent unrepaired break** is in scope — once the 48h closes, streak is permanently 0 and subsequent misses have nothing to repair. **Schema additions:** `streak_shields.used_at` + `usage_type` (`'proactive_auto'` / `'retroactive_repair'`) + `repaired_streak_date`; `daily_races.repair_window_starts_at` + `repair_window_ends_at`. Implementation lands in Phase 4 alongside the broader anti-cheat + score submission work. Net: ~32 → ~30 Appendix D opens. v1.49 — **Phase 4 hardening discipline locked: 4 new Appendix D items + `.gitignore` widened** (session 31b, 2026-05-16). Triggered by a senior-engineer forum post CD surfaced on the four mistakes that show up in every vibecode-built app: secrets in repo, broken RLS, no rate limiting, no error handling past the happy path. Audited current repo state — clean today on secrets (no `.env` in tree or history, no secret-shaped strings in tracked files) but Phase 4 schemas (`supabase/schema.sql` + `supabase/phase4_schema.sql`) have zero RLS statements and Edge Functions aren't specced for rate limiting. Locked four new Appendix D items as hard gates on Phase 4 work: **#54 secret management discipline** (no secrets in repo; `.env*` excluded with `.env.example` as the only template; service_role + RevenueCat secret keys server-side only; pre-commit audit grep; acceptance = clean public repo audit at Phase 4 close), **#55 RLS on every Phase 4 table — default-deny, explicit allow** (every table gets `ENABLE ROW LEVEL SECURITY` in the same migration; `auth.uid() = player_id` baseline for player-owned data; cross-player reads via narrow explicit policies; acceptance = two-test-player isolation test against all 7 player-owned tables), **#56 rate limiting on every public Edge Function** (60/min per authenticated UUID baseline + burst to 120/min; tighter 30/min for anonymous; server-side score dedup via unique session_id; Daily Race deduped per `(player_id, race_date)`; acceptance = scripted 10× load test returns clean 429s), **#57 error handling discipline** (every external call wrapped, no silent failures, exponential backoff for transients, Sentry capture for surprises, 5s/10s timeout defaults, offline graceful-degradation, never block the 60s round loop; acceptance = every Edge Function has a third-party-failure integration test). **Immediate fix shipped:** `.gitignore` widened from `.env*.local` to `.env*` with explicit `!.env.example` allow — closes the existing Expo-template default gap before any real secrets land. Net: ~28 → ~32 Appendix D opens (4 added as forward-looking discipline; not regressions). v1.48 — **Quick-wins Appendix D batch: 5 items closed** (session 31, 2026-05-16). Five small independent calls locked in one focused pass: **#16 push permission pre-prompt** — soft prompt fires after first Daily Race completion (context > earliness), copy `Don't make us yell. We'll only ping you for the good stuff.` with sublabel listing the three triggers (New Daily Race · Someone beat your score · Your league's last day), opt-out `Not now` triggers one polite re-prompt after 5 more sessions then permanent silence. **#18 Pro receipt grace** — 24h. Transient RevenueCat failures don't immediately revoke Pro; silently re-check on next launch + cache last-known-good entitlement timestamp; surface a small non-intrusive `verifying purchase…` badge in Profile only after 12h of continuous failures; hard-lock at 24h; successful re-validation clears the badge silently (no celebratory message for what was never visibly broken). **#23 League week-one** — leagues form at launch minute (first 30 sign-ups close a league, next 30 form the next); first promote/demote fires on the first Monday 6am ET that is ≥4 days after launch (no 1-day fake weeks); subsequent weeks are standard 7-day cycles; late joiners mid-week-one wait for the next-Monday opening rather than getting inserted into a mid-week league. Operational follow-up: target a Monday or Tuesday launch so week zero lands at a natural 5–7 days, which also aligns with the Monday press cycle and the Tue–Thu App Store featuring rhythm. **#27 Streak Shield bundle pricing** — `$0.99 / $1.79 / $2.49` for 1 / 2 / 3 packs. Mild bulk discount (save $0.19 on 2-pack, $0.48 on 3-pack) rewards stocking up without making the 3-pack feel close to Pro at $4.99. 3-pack fills to inventory cap. Store framing: `1 Streak Shield` / `2 Streak Shields` *(save $0.19)* / `Stock the vault — 3 Streak Shields` *(save $0.48)*. **#51 post-answer setTimeout iOS pause cosmetic bug** — formally deferred; single anecdotal occurrence, self-resolves in ~1s, fix path well-understood (wall-clock the post-answer advance, same shape as v1.43's round-timer fix). Revisit only if it becomes a reproducible pattern in beta; kill permanently at Phase 7 pre-launch polish if no new reports. Net: ~33 → ~28 Appendix D opens. One consolidated Decision Log row. v1.47 — **Result-screen back navigation fixed** (session 30, 2026-05-03). On device, returning to home from `/end` (Daily Race + Quickmatch result), from `/daily` alreadyPlayed view, or from `/echo` result view caused a half-second of visible overlap — the slide-out screen and home rendering simultaneously, with home's on-mount animations (Brain wiggle, ArcadeCard bob, panel pulse) firing from scratch. Cause: all four HOME buttons used `router.replace('/')`, which mounts a fresh `(tabs)` instance on top of the existing one rather than dismissing to it. Fresh mount = restarted animations. Fix: flipped four call sites to `router.back()` — pops the current screen and reveals the existing `(tabs)` underneath, where home is already mounted with animations already running. Native iOS back-pop animation, no remount, no double-paint. Comments added inline at each call site explaining the choice. The other `router.replace('/')` calls in the codebase (`echo.tsx:169`, `daily.tsx:110`, `game.tsx:50`) are gate-enforcement redirects and remain correct as `replace`. Pattern documented in Decision Log: result-screen dismissal uses `router.back()`, gate-redirect uses `router.replace()`. Verified on device. v1.46 — **Brain mascot load order fixed** (session 29, 2026-05-03). Hero brain on home was rendering its SVG face overlay (eyes + mouth, synchronous render tree) before the PNG body (`brain-base.png`, 1.27MB, async decode), which read on device as "face floating in air for ~100–300ms → pink body pops in." Two-part fix: (1) `app/_layout.tsx` now preloads the brain PNG via `expo-asset`'s `Asset.loadAsync()` in parallel with fonts + AsyncStorage hydration — splash holds until all three are ready, so the decode is hoisted into the splash window where the user can't see it; (2) `components/Brain.tsx` adds an `imgLoaded` state defaulting false, set true on the Image's `onLoad`, with the SVG face overlay wrapped in `{imgLoaded && <Svg>...</Svg>}` — defense-in-depth so the face never paints before the body even if a re-mount races the preload. Code change is small (~15 lines net). Pattern documented in Decision Log: hero/identity image assets get preloaded at boot alongside fonts; the splash window is the right place for any decode that would otherwise show as a paint glitch on home. Other Brain assets are vector — only the body PNG needed preloading. Verified on device (cold launch + warm relaunch). v1.45 — **Day-boundary drift fixed; clock anti-tamper deferred to Phase 4** (session 28, 2026-05-03). Shipped a small correctness fix that ripples across the once-per-day lockout and the Daily Race seed. New shared helper `app/clock.ts` exposes `getRaceDate()` (returning the 6am-ET-anchored calendar date `YYYY-MM-DD`, DST-aware) and `getRaceSeed()` (the same date as a stable integer seed). All six callsites that previously computed `today` from `new Date().toISOString().split('T')[0]` (UTC midnight) — `app/store.tsx` (`recordPlay` / `tapOneMore` / `setDailyPlayed`), `app/daily.tsx` (the lockout and the question-set seed), `app/(tabs)/index.tsx` (home freePlay gate + `dailyPlayedToday`), `app/echo.tsx` (Quickmatch freePlay gate), `app/game.tsx` (same) — now route through `getRaceDate()` / `getRaceSeed()`. Two correctness wins: (1) the once-per-day lockout now flips at 6am ET (matching the spec locked 2026-04-24), not UTC midnight (which would double-play in the 10-hour window between 8pm ET and 6am ET); (2) the Daily Race seed is now globally stable — players in Istanbul and NYC at the same UTC instant get the same question set, which the previous device-local seed broke (violating the equal-ground principle). Math verified against 7 boundary cases (5am ET / 6am ET boundary flip / 11:59pm ET / next-day 5:59am ET / DST and EST). Local `isUSDST` in `app/(tabs)/index.tsx` deleted; `useCountdownToNext6amET` now imports it from `app/clock.ts`. **Anti-tamper logic explicitly deferred to Phase 4** (Appendix D #6 extended): a player who rolls their device clock forward can still defeat the local lockout pre-Phase-4. Server-side submission validation is the canonical anti-cheat layer; this session is correctness, not anti-cheat. Beta + TestFlight testers will be told the cheat path exists. One-time migration leak: players whose `dailyStatus.date` was written under the old UTC-based code, during the 10-hour window when UTC date was one day ahead of race date, will see one extra Daily Race attempt the first time they open the app under the new code. Not worth fixing — Phase 4 reset normalizes. v1.44 — **Home-screen polish pass shipped** (session 27, 2026-04-27). Three home surfaces redesigned + ported to RN, all driven by the same `dailyPlayedToday` flag the rest of the app already reads. (1) **Hero**: speech bubble removed; brain promoted to 92pt; brain + wordmark vertically center-aligned. (2) **Mascot**: antenna retired everywhere — Brain.tsx now renders an illustrated PNG body (`assets/images/brain-base.png`, 1402×1122 transparent) with eyes + mouth as an SVG overlay drawn at the PNG's native viewBox (eye coords ported verbatim from `explore/shared.jsx`: cx 335/645, cy 540, rx 82, ry 100; pupils inward by 8×flip). New `expression="hype"` (open mouth + tongue) added for the YOU-row leaderboard avatar. Role split: top-left brain = "this is your home" (smirk, 92pt, wiggle); YOU-row brain = "this is you in the race" (hype, 32pt disc with tier ring, no wiggle). (3) **Daily Race card**: branches on `dailyPlayedToday`. Fresh state: `DailyDecor` target + `FRESH EVERY 6AM`. Done state: same `DailyDecor` shell but new green-ringed checkmark stamp behind the same 5s lock-on / cream-flash cadence (only the visuals under the ring swap — zero reflow), plus `650 PTS · #34 OF 1,247` and a small `BACK IN HH:MM:SS` countdown line. Card stays fully vivid in both states. ArcadeCard gained an optional `tertiary` text slot (small mono, tabular nums, dim) to host the countdown without growing the card. (4) **DailyRaceRankings panel** (new component) replaces the old `GlobalLeaderboard` widget on home — header `DAILY RACE RANKINGS · DAY 247` with pulsing yellow dot on ink, top 3 placeholder rows with monogram AvatarDisc, dashed divider, then either YOU row (highlighted yellow with brain avatar in newcomer-tier ring, hype expression, no wiggle) when played OR PlayToEnterRow (same height/density, brain avatar + `START →` pill) when fresh. Footer: `SEE FULL RANKING IN LEAGUE ↦` → routes to League tab. Same row chrome both states means zero reflow when daily flips. `GlobalLeaderboard` component file preserved untouched for League tab use; the OVERALL/DAILY tab strip + `412K` total counter retire from home (home preview is today-only; full leaderboard browsing lives on League). New `useCountdownToNext6amET` hook (DST-aware US-ET math, 1s tick) is the pre-Phase-4 fallback — Phase 4 swaps to a server boundary. New `Tier` palette added to `constants/theme.ts` (8 tier hex values from v1.32 — used as solid fills wherever a single tier color is needed; gradients still live on the avatar border component when Phase 3 ships it). Mocked until Phase 4 (Daily Race board endpoint): user's daily rank (`#34`), total players (`1,247`), top-3 row data, `DAY 247`. Fresh-state Daily Race card was spec'd + shipped but not personally tested on device (creative director's `dailyStatus.played` was already true at handoff); flips automatically at next 6am-ET reset. Two new Appendix D follow-ups (#52 server-driven daily rank/total + ranking refresh-on-focus; #53 brain avatar editor UI ship date). Three new Decision Log rows. Reference deliverables for the polish pass: `Home v2 — Fresh.html` + `Home v2.html` + `explore/home-v2.jsx` + `explore/shared.jsx` (Claude Design exploration). v1.43 — **Round lifecycle locked + Tier 1 #2/#3 closed** (session 26, 2026-04-26). Mid-round exits are terminal: navigation-away (back button, swipe-back) locks the score and ends the round; backgrounding (app switcher, notification, phone call) does NOT pause — round timer keeps running on wall-clock time, player loses time on return. Force-quit captures latest score via progressive saves on every answer (highScores for Quickmatch, dailyStatus for Daily Race). 200ms AsyncStorage debounce is the only vulnerability — physically impossible reaction window for a human, accepted as edge. Wall-clock-based timer (computed from `roundStart` timestamp, never decremented) makes the round timer robust to iOS lifecycle quirks (fixes the double-subtraction bug from app-switcher transitions). Daily Race component-local `currentlyPlaying` flag prevents the alreadyPlayed view from rendering mid-round even though dailyStatus.played flips to true on first progressive save. Resolves Appendix D #21. New Appendix D items: #50 back-button handler (✅ resolved this session), #51 post-answer setTimeout deferred during iOS pause (open — cosmetic, self-corrects). v1.42 — **Per-question rhythm revised** (session 25, 2026-04-26). Old pattern: 1-second visible UI lock per question + variable 800/1500ms post-answer animation. New pattern: 150ms **invisible** double-tap guardrail (below the ~200ms human reaction-time floor) + unified 1-second post-answer animation (right, wrong, and with-multiplier all use the same beat). Speed-bonus 2-second window starts at question render — no free read buffer. The 60-question set size stays unchanged: now sized for the spam-tap ceiling (~50 questions theoretical max in 60s) plus margin for future power-ups, rather than the earlier "60 questions / 60 seconds 1:1" framing. Code changes shipped to `app/echo.tsx`, `app/daily.tsx`, `app/game.tsx`. Verified on device. Game mechanics section in Part 3 + ghost-pool architecture in Part 4 updated. v1.41 — **Live Players Strip spec locked** (session 24, 2026-04-26). New Part 4 Layer 1.5 subsection covers the Quickmatch card footer strip end-to-end: "X PLAYING TODAY" with rolling-24h unique-player count, exact integers (no K/M rounding), server-cached cron-driven aggregation (decouples client traffic from DB load), 6 frames/min playback at 10s intervals with magnitude-scaled jitter (±1 at ≤50 up to ±5 at 1k+) for visible "trailing" motion, soft floor at <20 (random 20–30 display), stale-cache fallback for invisible server-failure handling, schema +1 column `players.last_seen_at`. Phase 4 implementation — ~3 hours of work when auth + Supabase wiring lands. v1.40 — **Tier 1 #1 closed: AsyncStorage persistence** (session 23, 2026-04-26). `app/store.tsx` wired to persist avatar, freePlay, dailyStatus, highScores via single-blob `@supersmart/state` key with `_version: 1` migration scaffolding + 200ms write debounce. Hydration runs in parallel with font load in `_layout.tsx`; render unblocks when both ready. **Closes the gate-bypass abuse path** + the once-per-day Daily Race loophole. Plus a home-screen UX win: when today's Daily Race is done, the card stays vivid cyan with `DAILY RACE ✓` label, score in sublabel, and the target accent flips red→green. Build-for-the-flex pattern (matches Wordle/Duolingo/Apple Fitness — bright + result-inline, never grayed). Resolves Appendix D #2. v1.39 — **Corpus-wide style sweep executed** (session 22f, 2026-04-26). Three clusters cleaned in one mechanical pass on 1001.xml + cascaded to app/questions.ts: **42 "Make a word using" prefix normalizations** (lowercase + single-space across the cluster); **4 math caps to lowercase** (Q738/Q739/Q744/Q777); **Q626 Roman numerals uppercased** (c/k/x → C/K/X). 49 total field changes. Phase 1 audit tally unchanged (903/98/0/0) — sweep is style normalization, not Light edits. Phase 1 fully closed; ready for Phase 5 question writing. v1.38 — **Incident runbook drafted + locked** (session 22e, 2026-04-26). New file `incident_runbook.md` at parent root (mirrored to `supersmart/docs/`) — 6 pre-launch scenarios, 5 canonical PostHog kill-switch flags, player-facing maintenance copy voice-locked. Cross-reference added to mothership Part 7 (Engineering & Stack) so the runbook is discoverable from here. Living document — postmortems update it after every actual incident. v1.37 — **1001.xml updated with all 98 Light edits applied** (session 22c, 2026-04-25). Original 2012 corpus preserved at `1001_original_2012.xml` (archive); new audited corpus is now the live `1001.xml`. Mechanical apply via structured edits dictionary; integrity verified (1001 questions, all answers match options, exactly 98 changes diffed against original = audit set). Phase 1 deliverables complete: tagged CSV ✓, methodology doc ✓, edited corpus XML ✓, original archive ✓. Ready for Phase 5 question writing against this voice corpus. v1.36 — **PHASE 1 AUDIT COMPLETE + FINAL-CHECK PASS** (session 22b, 2026-04-25). Final-check fresh-eyes sweep across all 1001 questions caught 5 more candidates: **Q71** (dinosaur extinction 65M → 66M for current K-Pg consensus), **Q321** (Smart German automaker → BMW; Smart is now Mercedes+Geely JV), **Q594** (Woody Allen → Christopher Nolan; Annie Hall → Inception; cultural drift ruling #16), **Q875** (Texas population threshold "<33M" → "<40M"; Texas growing toward 33M during game lifespan, ruling #14), **Q932** (rewrite to "larger country by population" Japan/Germany/France with Japan correct; resolves city/options mismatch + duplicate avoidance). **Final corpus-wide tally: 903 / 98 / 0 / 0 = 90.2% Keep / 9.8% Light / 0% Heavy / 0% Retire.** Per-category Keep rates: math 100%, science 82.5%, misc 88.7%, word 91.4%, music 84%, movies 86.7%, geography 91.3%, people 82%, literature 90%. Six batches across 5 working sessions (13, 14, 16, 19, 21, 22). **Zero Heavy or Retire flags across all 1001 questions.** The Part 8 projection of 700/150/100/50 was off by an order of magnitude on Heavy/Retire — the corpus is dramatically more durable than Phase 0 sampling suggested. Per-category final Keep rates: math 100%, science 81%, misc 89%, word 92%, music 86%, movies 90%, geography 92%, people 82%, literature 90%. Notable batch 6 fixes: Q868 equator-doesn't-pass-through-Argentina factual error → Brazil; Q924 Sears Tower → Willis Tower (already-effected rename, ruling #14); Q979 French goalkeeper Barthez → famous French footballer Mbappé (contemporary anchor); Q968 distractor Bradley Manning → Bradley Cooper (cultural drift, ruling #16); Q988 Pluto → Plato source-XML broken-puzzle fix; Q935 Everest China-vs-Nepal ambiguity → "in the Himalayas mountain range." **Phase 1 deliverable**: 1001-row tagged CSV (`audit_1001/audit_1001_tags.csv`), 16 edge-case rulings, methodology playbook, calibration table with per-category actuals. Next: corpus-wide style sweep before Phase 5 question writing. v1.34 — Phase 1 audit batch 5 closed + cultural-relevance sweep (session 21, 2026-04-25). 300-question mega-batch (Q401–Q700) — speed mode + a fresh-eyes cultural-drift pass over Q1–Q700 catching phrases whose meaning has shifted since 2012 (red pill manosphere baggage, Sarkozy fading political relevance). New methodology ruling #16 added for cultural-drift Lights. Final batch 5: **278 Keep / 22 Light / 0 Heavy / 0 Retire** = 92.7% Keep. Cumulative through Q1–Q700: **629 / 71 / 0 / 0** = **89.86% Keep**. **Five batches in (70% of corpus), still zero Heavy or Retire flags.** Dated-cluster forecast for music/movies materialized only mildly (86% / 90% Keep — well above 40–60% original forecast). Two durability-driven concept-replaces: Q543 Bieber→Swift answer ("Shake It Off"); Q595 Will Smith→Tom Cruise (Will Smith's 2022 Oscar win invalidated the "never won" answer — first **already-effected fact change** in the audit, methodology ruling #14 extended to cover both pre-announced AND already-effected changes). One Retire flag was raised on first pass (Q501 duplicate of Q326) but CD reframed to a Light-edit rewrite (group-of-goats puzzle), preserving the perfect zero-Retire streak. Math 100% Keep across 95 questions — the cleanest category yet. The 700/150/100/50 Part 8 projection is now decisively pessimistic; running rate suggests final around 900/100/0/0 if late categories cooperate. Remaining: 80 math + 161 geography + 50 people + 10 literature = 301 questions. People (Q942–Q991) is the last major attrition risk. v1.32 — League rank border palette locked (session 20, 2026-04-25). Appendix D #11 resolved. All 8 tier hex values + gradient structures + Legend shimmer execution specified in Part 3. Visual escalation: solid 1–5, 2-stop gradient at Finalist, 3-stop static gradient at Champion (mirrors Legend's recipe in crimson), 3-stop animated gold gradient at Legend (continuous low-intensity shimmer + theatrical-entry brighten on the promote-to-Legend interstitial). Two minor evolutions from the v1.25 directional spec: Newcomer "white" → pale dusty blue `#A8C4D8`; Finalist "orange" → magenta gradient. v1.31 — Phase 1 audit batch 4 closed (session 19, 2026-04-25). Q301–Q400 tagged at **90 Keep / 10 Light / 0 Heavy / 0 Retire** — same shape as batches 2–3. Cumulative through Q1–Q400: **351 Keep / 49 Light / 0 Heavy / 0 Retire** (87.75% Keep). First word category coverage: anagram cluster Q352–Q386 ran very clean (94% Keep across the 50-question word slice). Two retired-mode concept-replaces (Q331 Arcade→Quickmatch, Q332 Classic→Daily Race — same pattern as Q265 batch 3). One corpus-wide style sweep deferred to end-of-Phase-1: the "Make a word using  X" double-space prefix typo + Make/make capitalization split across Q352–Q386 (35 questions, identical fix). Three char-overflow Keeps approved by CD discretion against the ≤40 ceiling — methodology now has ruling #15 softening the rule to "default Light, CD-discretion override on small overruns where the fix degrades voice." **400 questions audited (40% of corpus); zero Heavy or Retire flags four batches in.** Part 8's 700/150/100/50 projection meaningfully pessimistic — would land closer to 875/125/0/0 if rate held, but people/music/movies depths still likely carry the dated cluster. Will revisit projection after batch 6. v1.30 — Post-league UX fully specced (session 17). Appendix D #26 resolved. Three result states locked: Promoted (theatrical border-color reveal, share button, `[Tier]. Yours now.`), Held (position-aware dry copy, three micro-brackets), Demoted (quiet, no reward, `Not this week.`). Full spec in Part 4 Layer 4. v1.29 — Phase 1 audit batch 3 closed. Cumulative through Q1–Q300: **261 Keep / 39 Light edit / 0 Heavy edit / 0 Retire** (87.0% Keep). Batch 3 (Q201–Q300, all misc) closed at 90/10/0/0 in session 16 — same shape as batch 2. Two questions hit "concept-replace" rather than tweak: Q265 reframed because its answer ("arcade") referenced a retired mode (same fix the code-side `app/questions.ts` got in CHANGELOG session 11), Q266 reframed because the iPod answer is dated post-2022 discontinuation. Six of ten Light edits this batch were proper-noun capitalization (months + names) — same pattern as batch 2. **Three batches in (300 questions, 30% of corpus), still zero Heavy or Retire flags.** Mothership Part 8's 700/150/100/50 projection now meaningfully pessimistic at the running 87% Keep rate. Will revisit projection after batch 4. v1.28 was the codebase + documentation audit pass (session 15, 2026-04-25). Five small drift items closed: (1) deleted 8 dead boilerplate files from the Expo template (themed-text/view, collapsible, parallax-scroll-view, 2 use-color-scheme hooks, use-theme-color hook, modal screen) — closed island of dead code that referenced a `Colors.light` / `Colors.dark` shape `theme.ts` doesn't have; if dark mode is reopened later it should be redesigned from scratch against a parallel palette, not by reactivating template scaffolding; (2) `app.json` `userInterfaceStyle` flipped from `automatic` to `light` (matches the locked light-only commitment); splash-screen `dark` background removed; splash backgroundColor changed to cream `#FFF4DF`; `app/_layout.tsx` modal route registration removed; (3) primer refreshed to current state, drops the "audit overdue" line, picks up batch 1+2 progress; (4) Part 5 free-tier rounds nailed from "5–7" range to **7** (matches the `FREE_LIMIT=7` PostHog flag default that was already in code); (5) Phase 7 launch window made explicit ("August 1–15, 2026"). Three new Appendix D code-side tracker items added (#47–49: IS_PRO hardcode, duplicate `getRankLabel`, Profile mailto domain dependency). v1.27 closed Phase 1 audit batch 2. Cumulative through Q1–Q200: **171 Keep / 29 Light edit / 0 Heavy / 0 Retire** (85.5% Keep). Batch 2 (Q101–Q200, last 20 science + first 80 misc) closed at 90/10/0/0 in session 14 — the misc slice ran much cleaner than forecast (89% Keep) because it's mostly self-referential wordplay, fourth-wall meta, family-tree/letter puzzles, and durable basics (colors, alphabet, days, rock-paper-scissors, historical Olympics). The dated-pop-culture cluster expected for misc must live deeper in the category (Q201+ inside misc, batches 3+). One new edge-case ruling locked into methodology: **#14 — time-bound facts with pre-announced changes are Light edits even if technically correct at launch** (squash → bowling for Q147 because squash was approved for LA 2028 Olympics in 2023). Calibration table updated: misc forecast revised, late-misc still expected to carry most of the Heavy+Retire load. Methodology doc also got a section on corpus-wide stylistic decisions (capitalization, underscore counts) — fix inline as you spot them, run one global sweep at the end of Phase 1 to catch residuals. v1.26 captured batch 1 close (Q1–Q100, all science, 81/19/0/0). v1.25 bundled 5 mini-decisions (league tiers 1–3 renamed with entry at tier 2, splash 2s every launch, emoji policy, question retirement three-tier system with "Contact the developer" email in Profile in place of in-app report button; #40 support domain deferred). League ladder now Rookie → Newcomer → Regular → Veteran → Qualifier → Finalist → Champion → Legend.
...
- ~~"Get super smart today by getting Super Smart today!" as the closing CTA~~ *— retired 2026-05-16 session 32; CD removed it from the home screen footer. The line stayed in the spec as long as the home rendered it; once removed from the product it leaves the doc too. Preserved struck-through for historical record.*
...
- **Resets daily at 6am Eastern Time.** *[DECIDED 2026-04-24]* Reset clock is anchored to `America/New_York, 06:00` — underlying UTC auto-shifts between 10:00 UTC (EDT) and 11:00 UTC (EST) twice a year. Supabase cron job configured in `America/New_York` tz. Stored behind the `daily_race_reset_time` PostHog flag (Part 7 Flexibility Architecture) so it can be retuned without a submission. **DST gotcha:** the reset-week is 23 or 25 hours long — the once-per-day lock must be built on the reset window, not a rigid 24-hour timer, otherwise players either double-play (25-hr week) or get locked out (23-hr week). **Code anchor** *[WIRED 2026-05-03 session 28]*: client-side day boundary computed via `app/clock.ts`'s `getRaceDate()` (returns the 6am-ET-anchored `YYYY-MM-DD`, DST-aware). `getRaceSeed()` is the integer form used to seed the daily question set so every player worldwide draws the same set during the same global 24h window. Both the once-per-day lockout (in `app/store.tsx` + `app/daily.tsx`) and the freePlay reset (in `app/echo.tsx` / `app/game.tsx` / home) route through this helper. Pre-Phase-4 fallback; trusts the device clock — anti-tamper layer (Appendix D #6) lives server-side at submission time.
...
- ~~The closing CTA stays: "Get super smart today by getting Super Smart today!"~~ *— retired 2026-05-16 session 32. See Part 2 strikethrough entry above. No replacement.*
...
| 2026-05-03 (session 30) | **Result-screen dismissal uses `router.back()`; gate-redirect uses `router.replace()`.** Two distinct navigation patterns clarified. **Result dismissal** (player tapped HOME from `/end`, from `/daily` alreadyPlayed, or from `/echo` result) → `router.back()`. Pops the current screen on the stack and reveals the existing `(tabs)` underneath. Home is already mounted there with animations already running, so the only thing animating during the transition is the iOS back-pop slide itself. **Gate redirect** (player hit a screen they shouldn't have access to — over freePlay limit hits at `echo.tsx:169`, `daily.tsx:110`, `game.tsx:50`) → `router.replace('/')`. Replace is correct here because the player should not have a back-stack entry pointing to the gated screen. Both patterns are now documented in inline comments at each call site. | The visible bug was a ~half-second overlap on device when returning to home from a result screen — slide-out and slide-in playing simultaneously while home's on-mount animations (Brain wiggle, ArcadeCard bob, panel pulse, Live Players Strip) restarted from zero. Diagnosis: `router.replace('/')` mounted a fresh `(tabs)` instance on top of the stack rather than dismissing to the existing one; fresh-mount fires every `useEffect` hook on home as if first-time. Fix is one method change per call site (four total), no architectural shift, no transition-config tuning needed — the iOS back-pop animation is already correct, we were just navigating in the wrong direction conceptually. Locking the pattern in the decision log so future result screens (challenge result, league interstitial, anywhere a player taps "go home" from a stacked screen) follow the same shape. |
| 2026-05-03 (session 29) | **Hero/identity image assets preload at boot — brain PNG load-order fix.** New pattern: image assets that are part of the brand-identity layer (hero mascot, wordmark when bitmap-backed, splash decor) get preloaded in `app/_layout.tsx` via `expo-asset`'s `Asset.loadAsync()` alongside fonts + AsyncStorage hydration. Splash holds until all three resolve. Specifically wired this session: `assets/images/brain-base.png` (the 1402×1122 PNG body of the Brain mascot, ~1.27MB). Without this, the synchronous SVG face overlay (eyes + mouth) painted before the async PNG decode completed, showing on device as "face floating in air → pink body pops in" for ~100–300ms on first home render. With preload, the decode is hoisted into the splash window. Defense-in-depth added in `components/Brain.tsx`: `imgLoaded` state gates the SVG face overlay (`{imgLoaded && <Svg>...</Svg>}`) so the face never paints before the body even if a re-mount races the preload. | The brain is the player's first visual contact with the brand on every cold launch, and a "broken-feeling" first frame undermines the polish the rest of the home screen does. Splash-window preload is the right architectural home for this class of fix — the splash already holds for fonts + state, and adding an image dependency costs ~150ms of decode that's already invisible to the user. The onLoad gate is cheap insurance against memory pressure or re-mount edge cases. Pattern generalizes to any future hero/identity asset (avatar editor color variants in Phase 3, splash decor PNGs if any ship). Vector assets (Wordmark, Sunburst, Halftone, mascot face) don't need preloading — they're synchronous in the render tree. |
| 2026-05-03 (session 28) | **Day-boundary drift fixed — client-side `today` now anchored to 6am ET via shared helper.** New module `app/clock.ts` exposes `getRaceDate()` (returns `YYYY-MM-DD` for the current Daily Race day, computed as the ET calendar date as of 6 hours ago — DST-aware via the existing `isUSDST` logic, now relocated here from `app/(tabs)/index.tsx`) and `getRaceSeed()` (the same date as a stable integer for the question-set RNG). Six callsites that previously computed `today` from `new Date().toISOString().split('T')[0]` (UTC midnight) and the Daily Race seed that previously used `new Date().getFullYear() * 10000 + ...` (device-local) now route through these helpers: `app/store.tsx`, `app/daily.tsx`, `app/(tabs)/index.tsx`, `app/echo.tsx`, `app/game.tsx`. Two correctness wins: (1) the once-per-day lockout now flips at 6am ET, matching the 2026-04-24 reset spec — previously the UTC-midnight key gave a 10-hour window (8pm ET → 6am ET) where the lockout flipped before the actual race day rolled, allowing two plays per "real" race day; (2) the daily question set is now globally stable — players in Istanbul (UTC+3) and NYC (UTC-5) at the same instant draw the same set, where the previous device-local seed gave them different sets and broke the equal-ground principle. Math verified against 7 boundary cases (5am ET / 6am ET flip / 11:59pm ET / next-day 5:59am ET / DST and EST). Typecheck clean. One-time migration leak: players whose `dailyStatus.date` was written under the old UTC-based code, during the 10-hour window when UTC date was one day ahead of race date, will see one extra Daily Race attempt the first time they open the app under the new code; not worth fixing — Phase 4 reset normalizes. | The drift was hiding in plain sight: spec locked 6am ET on 2026-04-24, but the lockout key and the seed both kept using device-clock date math from earlier sessions. Surfaced this session while reviewing Tier 1 #4 (system-clock anti-tamper) — the audit found two things were entangled, and only one was anti-cheat. Separating them lets us ship the correctness fix immediately (no architectural decisions needed, just a helper extraction) while consciously deferring the anti-cheat half (server validation in Phase 4). The seed bug specifically would have been embarrassing in TestFlight: the marketing copy and the entire Daily Race identity hinge on "everyone gets the same questions today" — having timezone-split seeds in beta would have surfaced as "why does Istanbul get different questions" the first day a non-US tester played. Helper-first refactor (one module, six callsites all reading from it) means the Phase 4 server boundary swap is a one-file edit when that lands. |
| 2026-05-03 (session 28) | **Tier 1 #4 (Daily Race system-clock vulnerability) deferred to Phase 4.** Local anti-tamper logic — clock-jump heuristics, `lastSeenWallTime` checks, network time anchors via Apple/Supabase HTTP `Date` headers — explicitly **not** shipped pre-Phase-4. The single canonical anti-cheat layer is server-side submission validation (Appendix D #6): the server is the authority on "what day it is" and rejects any Daily Race submission outside the active 6am-ET → 6am-ET window for the player's recorded join, plus the existing reasonableness checks (points-per-second bound, suspicious-pattern flagging). Until Phase 4 lands, the cheat path (roll device clock forward → replay Daily Race) is open. Beta and TestFlight testers will be told this exists upfront. Appendix D #6 extended to explicitly include device-clock manipulation as part of what the server-side validator must guard against. | Three options were considered for the pre-Phase-4 window: (A) defer entirely, (B) lightweight local deterrent only, (C) network anchor + local deterrent. CD picked A — clean architectural boundary, no transitional code to throw away in Phase 4, no false-positive risk for legitimate timezone travelers, and the testers-find-it-anyway risk is mitigated by simply telling them. The right place to enforce "what day is it" is the same place that enforces "is this score reachable" — both are submission-time decisions on the server, both rely on the same source of authority, and bundling them into one Edge Function is cheaper than two layers. Pre-Phase-4 the device-clock cheat is bounded in damage: leaderboard pollution at most, and Phase 4 server validation will reject any retroactive submissions when it ships. The boundary fix from this session (route #1 above) is correctness, not anti-cheat — it would have been needed regardless of the anti-tamper decision. |
...
| 2026-05-16 (session 32) | **Round screen translated from Claude Design + Ghost concept removed end-to-end + closing CTA dropped.** Three workstreams shipped in one device-verified pass. **(1) Round screen:** 16 new components under `components/round/` (RoundSunburst, OutsideInPulse, HalftoneOverlay, EdgeGlow, PlayerBlock, VsBadge, StreakChip, TimerPill, QuestionCard, AnswerButton, FeedbackPopup, Nudge, FullscreenLightning, UnstoppableBanner, RoundLayoutA, constants). Layout A "stadium" — tier-driven background atmosphere (sunburst speed-up, outside-in vignette pulse with `mixBlendMode: 'multiply'`, halftone pulse, edge glow on correct/wrong), HUD with VS strip + colored timer pill + streak chip, bordered question card with per-question fade-in, chunky 3D answer buttons with state-aware faces, sub-2s fullscreen lightning, 10-streak UNSTOPPABLE banner, onboarding nudges. Wired into `app/echo.tsx` (quickmatch, pink) + `app/daily.tsx` (dailyrace, cyan). State machines unchanged — round/* is the visual layer. One mid-session crash bug fixed: React rules-of-hooks violation in 7 components (`useAnimatedStyle` after early-return). **(2) Ghost concept removed:** 5 customer-facing strings missed by session 7's rename (`FINDING YOUR GHOST` matching title, `👻 GHOST WINS` loss headline, `NEW GHOST` replay button, profile sub-label, challenge body) — all renamed. CD extended scope to all internal code (`GHOST_NAMES → OPPONENT_NAMES`, `ghost → opponent`, every style name, all comments via bulk sed across 7 files). Glossary updated: `Ghost run → Recorded run`, `Bot ghost → Bot opponent`, `Baton pass` def stripped. Historical decision log entries keep original "ghost" wording (audit trail). **(3) Closing CTA removed:** `Get super smart today by getting Super Smart today!` voice-bearing line removed from home footer; Part 2 + primer Voice Cheat Sheet updated to match. | Spec/code gap had been growing since session 5 (April 19, 27 days ago) when the 3-layer feedback system was designed but never coded — surfaced this session when CD tested current build and didn't see the expected visuals. The translation was its own work session per the `feedback_design_to_code_translation_gap.md` memory rule. Visual iteration on device was extensive (multiple pulse opacity tunings, color stop adjustments to anchor the colored ring within the screen at peak scale, transition width widened from 20% to 30% to soften the visible elliptical boundary, continuous sine-wave loop replacing discrete `withSequence` to remove the "hits hard each cycle" feel, edge glow z-index moved BEHIND the question card + buttons since RN can't reproduce the design's CSS blur). The ghost rename was originally session 7 scope; the 5 missed strings + the internal extension is what's locked here. Important precedent for future doc/code drift: don't rename in customer-facing copy only — extend through the codebase so the spec and the implementation use one vocabulary. |
| 2026-05-16 (session 31e) | **Launch-prep ops cluster parked — 4 items deferred with directional context locked.** #38 PR strategy → revisit ~4–6 weeks pre-launch (mid-July 2026); #39 beta tester list → revisit at beta recruitment time (CD already has the friend list); #41 App Store screenshots brief → revisit late Phase 6 (production needs final game assets); #42 IAP sandbox testing → revisit at TestFlight beta launch (Phase B). Two strategic context updates surfaced and locked: **(a) WhatsApp group replaces Discord** as the beta feedback channel — CD's existing contacts already there, zero adoption friction. **(b) Two-phase beta plan locked:** Phase A Expo Go (visual/UX feedback, can start today, no IAP testable) → Phase B TestFlight (~4–6 weeks pre-launch, full #42 sandbox plan applies). Each parked item retains directional context from the session 31e discussion so future revisits don't re-litigate the easy 80% of the decisions. | Parking is a discipline call at 12 weeks out, not avoidance: specifics on launch-week items get demonstrably sharper closer to launch (PR contacts shift, App Store assets need polished game to film, beta testers need an actually-testable IAP flow). The Expo Go insight is the meaningful unlock — it means CD can start sharing with friends today on the existing build, getting visual/UX feedback throughout Phase 3 without waiting for Phase 4 + TestFlight. The WhatsApp/Discord shift is small but load-bearing once it propagates to #39 + #42 — friend-circle beta testing works through the channels people already use, not channels they have to adopt. The discipline shown here (lock directional context inside the park notes so the deferral isn't "we'll figure it out later" but "we know mostly what we'll do, just need final timing-sensitive inputs") is the playbook for every future park. |
| 2026-05-16 (session 31d) | **Streak Shield pricing reversed — 3-pack removed; 1+2 packs repriced.** Same-day reversal of the v1.48 #27 lock (`$0.99 / $1.79 / $2.49` mild-discount three-SKU model) → new model is **`$1.99` (1-pack), `$3.49` (2-pack), no 3-pack**. Strategic intent: at $3.49 the 2-pack sits $1.50 below Pro at $4.99 — the natural upsell moment where "for $1.50 more, get Pro" makes the upgrade obvious. Removing the 3-pack eliminates structural redundancy with Pro's weekly shield grant (both SKUs were competing for "stock-up" intent). #19's retroactive repair UX updated inline to reflect the 2-SKU menu. New Phase 5/6 UX opportunity flagged (Pro upsell card at the 2-pack purchase moment) but not blocking. Original v1.48 pricing preserved in #27 under "Superseded v1.48 lock" for audit trail. | The v1.48 lock was made with "impulse-purchase accessibility" as the dominant rationale — $0.99 1-pack felt cheap enough to be a no-brainer after a streak loss. CD revisited within hours and shifted the strategic axis from impulse accessibility to Pro-funnel aggression: better to extract more revenue per shield buyer AND push more of them toward Pro than to keep shields cheap. The math case for the reversal: a player buying $1.79 in shields five times a year ($8.95 total) instead nets $9.95 plus a high probability of one Pro upgrade — better LTV economics. Removing the 3-pack specifically was the bigger insight: it was always the SKU competing most directly with Pro (both are "stock-up to cap" intent), and forcing the comparison at the 2-pack/$3.49 → Pro/$4.99 gap is structurally cleaner than offering both. Worth noting the discipline: reversal happened in the same session as the original lock, decision-log audit trail preserved with strikethrough rather than overwritten, no architectural code needed to change. Cheap to revisit pre-Phase-6; will be expensive after IAP products are registered in App Store Connect and historical purchase analytics start anchoring price expectations. |
| 2026-05-16 (session 31c) | **Streak Shield system fully specced — #17 Pro weekly auto-grant + #19 retroactive 48h repair closed.** #17: Monday 6am ET anchor (same clock as Daily Race + League reset), skip silently at the 3-shield cap, immediate welcome-grant on first-ever Pro upgrade then weekly cadence, refund/rebuy cycles don't re-fire the welcome grant (tracked via `pro_entitlements.first_granted_at`), retroactive grant on next app open if a Monday was missed (cap-aware, only most-recent missed Monday qualifies), pull-pattern Edge Function `check_pro_shield_grant` (rejecting cron-push for ops simplicity). #19: 48h window measured from start-of-next-day 6am ET (the moment the streak officially breaks, lines up with player discovery on next-day open), server-authoritative detection on Daily Race submit AND app open via new `check_streak_state` Edge Function, window only opens if 0 shields at miss time (proactive auto-activate takes precedence for shield holders per the 2026-04-19 spec), persistent in-app banner with countdown + push at T+4h and T+40h, only most-recent unrepaired break in scope (no multi-day backlog state), standard #27 pricing applies with 1-pack highlighted as repair / multi-packs as "stock up." Schema deltas: `streak_shields.used_at` + `usage_type` + `repaired_streak_date`; `daily_races.repair_window_starts_at` + `repair_window_ends_at`; `pro_entitlements.first_granted_at` + `last_granted_at`. | Closes the entire Streak Shield feature spec: design (2026-04-19 session 6) + pricing (#27, v1.48 earlier today) + Pro grant (#17) + retroactive repair (#19) all locked in one coherent system. Now ready to implement in Phase 4 alongside the broader anti-cheat + score submission work (#6) without architectural questions left open. The 6am-ET-everywhere discipline (one clock for Daily Race / League / Pro shield grants / repair windows) is paying its dividend here — every time-sensitive system in the app reads from the same anchor via `app/clock.ts` (extracted v1.45 session 28), so there's no second timezone math to test and no DST surprises. The pull-pattern decision for #17 also generalizes: any future Pro perk that's "1/week" or "1/day" should follow the same Edge-Function-on-open pattern rather than a cron job. |
| 2026-05-16 (session 31b) | **Phase 4 hardening discipline locked — 4 new Appendix D items.** Four mistakes that show up in every vibecode-built app (per a senior-engineer forum post CD surfaced) now coded as hard Phase 4 gates: #54 secret management discipline (no secrets in repo, `.env*` excluded with `.env.example` template, service_role + RevenueCat secret keys server-side only, pre-commit audit grep), #55 RLS on every Phase 4 table (default-deny + explicit allow on every migration, two-test-player isolation acceptance test against all 7 player-owned tables), #56 rate limiting on every public Edge Function (60/min per authenticated UUID baseline, burst to 120/min, anonymous tighter at 30/min, score submissions server-deduped, Daily Race deduped per `(player_id, race_date)`), #57 error handling past the happy path (every external call wrapped, exponential backoff for transients, Sentry capture for surprises, 5s/10s timeout defaults, never block the 60s round loop, every Edge Function has a third-party-failure integration test). Immediate fix shipped: `.gitignore` widened from `.env*.local` to `.env*` with explicit `!.env.example` allow. Audit confirmed current repo is clean (no secrets in tree or history, no secret-shaped strings in tracked files). | Visibility is the whole game with this class of mistake — they don't surface until they break in production. Coding them as numbered Appendix D items (rather than buried in a "security best practices" doc) makes them reviewable per-item with explicit acceptance tests. The four items also map cleanly to existing Phase 4 work: #54 to the auth-architecture decision (#1), #55 to the same plus the schema work, #56 to the anti-cheat work (#6), #57 to the offline-behaviour spec (#3) + Sentry instrumentation. So they don't add net Phase 4 workload — they constrain the shape of work that's already on the schedule. Acceptance tests are the load-bearing part: without them, "we'll be careful" is the same as "we won't notice." |
| 2026-05-16 (session 31) | **Quick-wins Appendix D batch — 5 items closed in one focused pass.** #16 push permission pre-prompt (fires after first Daily Race completion; copy `Don't make us yell. We'll only ping you for the good stuff.` with triggers sublabel; `Not now` → one re-prompt at +5 sessions → permanent silence); #18 Pro receipt grace period (24h hard-lock threshold, silent re-check on launch, non-intrusive `verifying purchase…` badge at 12h, badge clears silently on re-validation); #23 League week-one (leagues form at launch minute by sign-up order in 30-player blocks, first promote/demote on first Monday ≥4 days after launch, late mid-week-one joiners deferred to next-Monday opening, operational follow-up to target a Mon/Tue launch date); #27 Streak Shield bundle pricing (`$0.99 / $1.79 / $2.49` for 1/2/3 packs, mild bulk discount, 3-pack fills cap, framing copy locked); #51 post-answer setTimeout iOS pause cosmetic bug (formally deferred; revisit only if reproducible in beta; permanent kill at Phase 7 if no new reports). | Five small calls that were each a single CD call rather than multi-turn architecture work — perfect cluster for a batched session. Bundling avoids five separate mothership writes for low-context items and creates measurable momentum on Appendix D before tackling the heavy Phase 4 cluster (auth, offline, identity, anti-cheat). Each resolution unblocks a downstream Phase 4–6 implementation slot: #16 needs the push tokens table + a server endpoint, #18 needs the RevenueCat webhook handler, #23 needs the league formation Edge Function, #27 needs IAP product registration in App Store Connect. Net Appendix D opens: ~33 → ~28. Strategy moving forward: clear the easy independent calls first, then dedicate full sessions to the architecture-heavy cluster. |
...
- **Recorded run** — a recorded Quickmatch session (answer sequence + timings + score) stored against a question set. What future players race against. *Renamed from "Ghost run" 2026-05-16 session 32 as part of the end-to-end ghost-concept removal. The `ghost_pool` Phase 4 table name is a legacy of the original term — rename optional during Phase 4 build.*
- **Baton pass** — the opponent-selection rule: each player races the most recently completed recorded run on their set, then their own run becomes the next opponent for the player after them.
...
- **Bot opponent** — single-use, ephemeral opponent generated by the matchmaking Edge Function when no human recorded run exists at the matched player's skill tier on the assigned set. Indistinguishable from human opponents in the UI. Score range 300–3,000; avatar drawn from the full library (including Pro-locked items, for soft exposure); name follows no single pattern. Not persisted in `ghost_pool` (table name is legacy, see Recorded run above). Never enters a League of 30, never contributes to any leaderboard. Quickmatch-only. Locked 2026-04-24, renamed from "Bot ghost" 2026-05-16 session 32.
...
6. **Anti-cheat / score integrity.** Global leaderboards matter. What prevents a modified client submitting fake scores? Typical tools: server-side reasonableness checks (points-per-second bound), rate limiting, suspicious-pattern flagging. Not specced. **Includes the Daily Race system-clock cheat path** *[scope extended 2026-05-03 session 28]*: a player who rolls their device clock forward can defeat the client-side once-per-day lockout pre-Phase-4 and replay Daily Race for a better score. Local anti-tamper deterrents (clock-jump heuristics, `lastSeenWallTime`, network time anchors) considered and explicitly deferred — server-side submission validation is the canonical layer for this whole class of attack. Server must reject any Daily Race submission outside the active 6am-ET → 6am-ET window for the player's recorded join; combine with the points-per-second bound + suspicious-pattern flagging above. Beta + TestFlight testers told the cheat path exists pre-Phase-4.
...
16. ~~**Push notification permission UX.**~~ ✅ **RESOLVED 2026-05-16 session 31.** Soft pre-prompt pattern locked. **Trigger:** fires after the first Daily Race completion (not at onboarding — context > earliness; the player has just experienced the loop, so the value is concrete). **Copy:** `Don't make us yell. We'll only ping you for the good stuff.` with a sublabel listing the three actual triggers (`New Daily Race` · `Someone beat your score` · `Your league's last day`) so the player knows exactly what they're saying yes to. **Opt-out:** tapping `Not now` triggers one polite re-prompt after 5 more sessions (the soft retry), then permanent silence — respect the no. Tapping `Sure` fires the iOS system dialog immediately. The iOS dialog itself is only ever invoked once, after a soft `Sure`, so a denial there is on the player rather than a wasted prompt. Implementation: Phase 4 alongside the push tokens table; PostHog flag `push_pre_prompt_enabled` defaults true with the copy hardcoded as fallback.
17. ~~**Pro weekly Streak Shield auto-grant.**~~ ✅ **RESOLVED 2026-05-16 session 31c.** Implementation pattern + 5 edge cases locked.

    **Grant clock:** Monday 6am ET — same anchor as Daily Race + League reset (#7). One clock for the whole system; no second timezone math.

    **At cap (3 shields held):** skip the grant silently. No carryover, no banked credit. The Pro perk is "you tend to have shields available," not "you accumulate infinite shields"; a player at cap is clearly not actively using them.

    **Mid-week Pro upgrade:** immediate "welcome to Pro" grant on first-ever upgrade as a feel-good moment, then next regular Monday grant fires as scheduled. A Tuesday upgrader gets 2 shields in their first week (1 on upgrade Tue + 1 on next Mon).

    **Refund + re-upgrade cycles:** one-time welcome grant per account ever, tracked via a `pro_entitlements.first_granted_at` timestamp. A refund-and-rebuy cycle does NOT re-fire the welcome grant — prevents shield-farming via repeated buy/refund.

    **Missed Monday (player not active that week):** retroactive grant on next app open within reach of the missed Monday. Cap-aware: only the most recent missed Monday gets a retroactive grant, never stacks past 3. A two-week-inactive player who returns Wed gets 1 grant (the most recent Monday), not 2.

    **Implementation: pull pattern via Edge Function `check_pro_shield_grant`** — chosen over cron-push for three reasons: (a) no cron job to monitor / page on failure; (b) inactive users naturally cost nothing (no grant fires for someone who doesn't open the app); (c) retroactive grants come for free with the same code path. Function called on app open + on Pro upgrade. Reads `pro_entitlements.first_granted_at` and `pro_entitlements.last_granted_at`; if current time is past a Monday 6am ET that is after `last_granted_at`, insert a `streak_shields` row (subject to cap) and update `last_granted_at` to the most recent reset window. Schema delta: `pro_entitlements.first_granted_at TIMESTAMPTZ`, `pro_entitlements.last_granted_at TIMESTAMPTZ`.
18. ~~**Pro receipt validation failure grace period.**~~ ✅ **RESOLVED 2026-05-16 session 31.** **24-hour grace** with a non-intrusive UX path. RevenueCat validation failure does NOT immediately revoke Pro entitlement. Instead: silently re-check on every app launch + cache last-known-good entitlement timestamp; if failures persist past 12 hours, surface a small `verifying purchase…` badge in the Profile tab (no Pro feature degradation yet — the player still gets unlimited rounds, seasonal packs, all Pro perks); only after 24 continuous hours of failed validation does the entitlement hard-lock. Once validation succeeds again, the badge disappears silently — no celebratory message for what was never visibly broken. Rationale: $4.99 one-time Pro means refund-fraud cost is small; "Pro disappeared on me" cost is huge reputationally. 24h covers basically every transient infra issue (RevenueCat blips, Apple receipt service slowness, network flakes). Implementation: Phase 4 alongside the RevenueCat webhook handler. Constants behind PostHog flags `pro_grace_period_hours` (default 24) and `pro_badge_threshold_hours` (default 12) so we can tune from server if early data shows otherwise.
19. ~~**Streak Shield retroactive 48-hour clock anchor.**~~ ✅ **RESOLVED 2026-05-16 session 31c.** Anchor + detection mechanism + UX during window + edge cases all locked.

    **Which reset the 48h is measured from:** the **start-of-next-day 6am ET reset** — i.e. the moment the streak officially broke, not the start of the missed day itself. Example: player missed Thu's Daily Race; the streak officially breaks at Fri 6am ET; the repair window is Fri 6am ET → Sun 6am ET. This lines up with how players discover the miss (on next-day app open) and gives them a full ~48h of conscious-window rather than ~24h of conscious-window + 24h they didn't know about.

    **Miss detection:** server-authoritative, runs on both: (a) Daily Race submission attempt (existing `submit_daily_race_score` Edge Function checks contiguity with `daily_races.last_completed_race_date`; if gap > 1 race-day, streak resets and repair window opens); (b) app open via new Edge Function `check_streak_state` (catches the case where the player doesn't even start a race the next day — opens the app Friday morning to find their streak broken without having tried).

    **Window only opens if 0 shields at miss time:** if the player held ≥1 shield, the proactive auto-activate rule (decision log 2026-04-19 session 6) takes precedence — the shield is silently consumed to preserve the streak. The retroactive 48h is a recovery path for players who weren't holding a shield. (Players who had a shield and used it proactively don't see a banner, don't get a push, don't see the window — there was no break to repair.)

    **UX during the 48h window:**
    - **Persistent in-app banner** above the Daily Race card with live countdown: `Repair your 12-day streak — 36h left.` (The `12-day` count is the broken-streak length, not the new one.)
    - **Push notifications** (assuming push permission per #16): one at T+4h (~10am ET morning after miss, after morning routine, before lunch attention drops); one at T+40h (final 8h reminder).
    - **Tap to repair** opens a focused purchase sheet at standard #27 pricing (`$1.99` 1-pack / `$3.49` 2-pack as of v1.51) — 1-pack highlighted as "repair the streak," 2-pack as "repair it and stock up." 2-pack purchases activate 1 shield retroactively; the second goes into inventory at cap. (No 3-pack — removed in v1.51's pricing reversal; Pro is the path to fully-stocked inventory.)
    - **After 48h expires:** banner disappears, streak permanently set to 0. No grace, no late repair.

    **Only the most recent unrepaired break is in scope.** If the player misses Thu and doesn't repair, by Fri 6am the streak is permanently 0. If they miss Fri too, there's no streak to repair — the system never enters a "multi-day backlog" state. Side benefit: math stays simple, no retroactive-stacking edge cases.

    **Retroactive purchase effect:** server inserts the `streak_shields` row, marks it used immediately with `usage_type='retroactive_repair'` and `repaired_streak_date=<missed day>`, restores `streak_count` to its pre-miss value, advances `last_completed_race_date` to the missed day. Client animation: streak count animates back up with a satisfying click; shield-in-inventory icon depletes by 1 if multi-pack was purchased.

    **Schema deltas:**
    - `streak_shields` table: add `used_at TIMESTAMPTZ`, `usage_type TEXT` (enum `'proactive_auto' | 'retroactive_repair'`), `repaired_streak_date DATE`
    - `daily_races` table: add `repair_window_starts_at TIMESTAMPTZ`, `repair_window_ends_at TIMESTAMPTZ`
    - New Edge Function `check_streak_state` (callable on app open); `submit_daily_race_score` extended with repair-aware logic.
...
23. ~~**League week-one launch experience.**~~ ✅ **RESOLVED 2026-05-16 session 31.** **Leagues form at launch minute.** First 30 sign-ups close a league together (all at tier 2 Newcomer per the universal-entry rule); next 30 form the next league; etc. The League tab is live with real cohorts from minute one — no "hollow leagues start Monday" placeholder state for early adopters. **First promote/demote fires on the first Monday at 6am ET that is at least 4 days after launch** — this ensures week zero is always 4–10 days, never an absurd 1-day fake week. Subsequent weeks are standard 7-day Monday-to-Monday cycles. **Late mid-week-one joiners** (anyone signing up after their league has already filled to 30) **wait for the next-Monday opening** rather than getting inserted into a mid-week league they didn't compete in — fair to existing members, clean cycle boundary for the new player. **Operational follow-up:** target a Monday or Tuesday launch date so week zero lands at a natural 5–7 days. Monday/Tuesday also aligns with the standard press cycle (Monday) and Apple's Tue–Thu App Store featuring rhythm — pure operational win. Implementation: Phase 4 league formation Edge Function reads a `launch_date` constant + applies the ≥4-day rule to seed the first promote/demote schedule.
...
27. ~~**Streak Shield bundle pricing.**~~ ✅ **RESOLVED 2026-05-16 session 31** → **REVISED 2026-05-16 session 31d** (same day; reversed within hours of original lock after CD revisited the strategic Pro-positioning).

    **Current pricing:** **`$1.99` for the 1-pack, `$3.49` for the 2-pack. 3-pack removed.** Two SKUs only.

    **Strategic intent:** at $3.49, the 2-pack sits $1.50 below Pro at $4.99 — the natural upsell moment where "for $1.50 more, get unlimited rounds + seasonal packs + weekly free shield + Pro avatar items + Global all-time view + Pro badge" makes Pro the obvious step up. Removing the 3-pack eliminates the structural redundancy with Pro's weekly shield grant (the 3-pack was the "stock-up" SKU; Pro is the "never think about it again" SKU — both were competing for the same purchase intent).

    **Three execution options considered before locking:** (A) keep 1+2 cheap at $0.99/$1.79 and jump 3-pack to $4.99 as a Pro-decoy — rejected (strictly-dominated math: 3× $0.99 = $2.97 < $4.99; dark-pattern risk); (B) shift entire curve up to `$1.99 / $3.49 / $4.99` with 3-pack matching Pro — rejected (3-pack still structurally redundant with Pro's weekly grant); (C) remove 3-pack entirely with the new prices — chosen.

    **Store framing in voice:**
    - `1 Streak Shield` — $1.99
    - `2 Streak Shields` *(save $0.49)* — $3.49

    **Superseded v1.48 lock (preserved for audit trail):** original resolution was `$0.99 / $1.79 / $2.49` for 1 / 2 / 3 packs with mild bulk discount. Rationale at the time: "shields are an impulse purchase right after a streak loss; bundles reward the shield-only power user without cannibalizing Pro upgrades." Reversed because the mild-discount approach left Pro looking like just "the better deal" rather than "the obvious step up at this price tier" — CD prioritized aggressive Pro funnel over impulse-price accessibility.

    **Downstream UX implication for #19:** retroactive shield repair purchase sheet now shows only the 1-pack and 2-pack — updated inline in #19's UX section. The 2-pack framing on the repair sheet stays "stock up" since a player who just lost a streak and is buying 2 shields is signalling intent to not lose another.

    **New Phase 5/6 UX opportunity (flagged, not blocking):** fire a Pro upsell card at the 2-pack purchase moment — "Buying shields? Pro covers this and more for $1.50 more." Track under future Phase 5/6 work; not a v1 launch blocker.

    **Out of scope of this decision** (flagged separately): whether free users can *earn* shields beyond the 1-at-launch grant (e.g. milestone unlock at 30-day streak) — that lives under Appendix D #9 (avatar milestone unlocks) and Phase 3 design work.
...
38. **Launch date PR strategy.** Warm-list press (148Apps, GameviewTonton), nostalgia angle, timing. ⏸ **PARKED 2026-05-16 session 31e — revisit ~4–6 weeks pre-launch** (mid-July 2026 for the Aug launch window). Specifics get sharper closer to launch: embargo timing, exact 20-outlet list, story-angle copy, founder long-form draft, materials kit. **Already directionally locked from the session 31e discussion** (recorded here for the future revisit): warm-list ~20 outlets with personal CD emails (no PR firm); embargo 1 week pre-launch with reviews dropping on launch day; story angle = nostalgia-led / product-substantiated; materials = press release + 5–7 screenshots + 30s preview video + founder long-form on Medium/Substack; iOS/games press only (defer mainstream tech to v1.1 milestone). **Pre-launch low-effort move that can start now:** Twitter follows + occasional comments on coverage from the target outlets (relationship-warming costs nothing, lands the outreach better when it ships).
39. **Beta tester list.** 10–20 friends for TestFlight. Names + coverage (iOS + Android, country mix). ⏸ **PARKED 2026-05-16 session 31e — revisit at beta recruitment time** (~2–3 weeks before first TestFlight build ships in Phase B; see #42's two-phase beta plan). CD already has the friend list assembled — no need to formalize today. **Directionally locked from the session 31e discussion** (for future revisit): target 15–20 testers; coverage minimums (10 iOS / 5 Android if Android ships at launch / 5+ outside North America for 6am-ET-reset timezone testing / 3+ trivia non-natives / 2+ over 50 / 1+ who played the 2012 original); weekly TestFlight build cadence during Phase B; **feedback channel = WhatsApp group** (CD's call — replaces the Discord assumption from earlier drafts); two-phase plan locked in #42 (Phase A Expo Go now → Phase B TestFlight ~4–6 weeks pre-launch).
...
41. **App Store screenshots + preview video creative brief.** Deliverable listed, no brief. ⏸ **PARKED 2026-05-16 session 31e — revisit late Phase 6** (early July 2026 for the Aug launch window). Production needs final game assets to film — brief comes alive when Phase 3 design polish + Phase 4 backend wiring are far enough along to produce polished gameplay capture. **Directionally locked from the session 31e discussion** (for future revisit): 8 screenshots (not 10 — leaves room for v1.1 updates); voice-y two-line captions (top line hook, bottom line proof point); hero = home screen (most recognizable identity surface); 30s preview video, gameplay-driven (not animated promo), recreate the energy of the recovered 2012 launch video; English only at launch per #46 localization decision. Screen order draft (subject to v1.1 product changes): [1] Hero/home, [2] Quickmatch result with high score + win animation, [3] Daily Race result with 60-square share grid, [4] League board, [5] Avatar customization, [6] One More moment, [7] Question screen mid-streak with multiplier visible, [8] Pro Wall.
42. **App Sandbox IAP testing strategy.** RevenueCat Sandbox mode + Apple Sandbox testers. Flow for beta testers to test purchase without real charges. ⏸ **PARKED 2026-05-16 session 31e — revisit at TestFlight beta launch (Phase B)** (~4–6 weeks pre-launch, when Phase 4 backend + IAP wiring lands). **Two-phase beta plan locked from the session 31e discussion** (load-bearing for #39 + #42 + downstream IAP work):
    - **Phase A — Expo Go beta.** Can start immediately (today, against the current build). Friends scan a QR code, install Expo Go from the App Store, run the game in minutes. Free, fast, no Apple Developer account required for testers. Useful for visual / UX / gameplay / "does this feel good" feedback throughout Phase 3 polish. **Cannot test IAP, push notifications, or any real native module** — Expo Go runs in a sandboxed environment that doesn't have StoreKit or RevenueCat's real IAP flow. Cannot test the Pro upgrade flow or Streak Shield purchases here. WhatsApp group is the feedback channel.
    - **Phase B — TestFlight beta.** Kicks in ~4–6 weeks pre-launch, once Phase 4 backend + IAP are wired and the Pro upgrade / shield purchase flows actually exist. The full #42 sandbox testing plan applies here.

    **Directionally locked from the session 31e discussion** (for future revisit of the Phase B plan): CD provisions 1 Apple Sandbox account per beta tester via App Store Connect → Users and Access → Sandbox Testers, sends credentials via WhatsApp DM; standard tester device setup (Settings → App Store → Sandbox Account → sign in); 6-test checklist pinned in the WhatsApp group (Pro upgrade flow / 1-pack purchase / 2-pack purchase / refund-and-rebuy welcome-grant verification / Restore Purchases on second device / One More × 3 → Pro Wall → upgrade); **clock-roll exploit enables #17 and #19 testing pre-Phase-4 anti-cheat** (testers roll device clock forward 2+ days to simulate missed-day state); **Pro grace period (#18) defers to post-launch internal testing** — Sandbox can't simulate RevenueCat validation failure cleanly without a debug toggle, not worth building for v1. **Phase 6 IAP launch checklist gotchas** (not decisions, but specced for future reference): product IDs in App Store Connect must match RevenueCat product IDs exactly; entitlement key in app code (`pro`) must match RevenueCat dashboard; first Sandbox purchase per account takes 30–60s to validate (Apple's sandbox is slower than production); Sandbox accounts can't make production purchases — keep isolated.
...
51. **Post-answer setTimeout deferred during iOS lifecycle pause** — buttons may briefly appear stuck/grayed mid-round. *Identified 2026-04-26 session 26.* ⏸ **FORMALLY DEFERRED 2026-05-16 session 31** — revisit only if it becomes a reproducible pattern in beta. When the app transitions to `inactive` (app switcher, brief notification pulldown) right after the player answers a question, the unified 1-second post-answer `setTimeout` can be deferred by iOS until JS resumes. During the deferral, `selectedAnswer` is non-null → buttons render in `correct/wrong/dim` state (visually grayed) → buttons are disabled. Self-corrects when JS resumes (the deferred setTimeout fires, advances the question, re-enables buttons). Same root cause as the round timer issue (resolved via wall-clock refactor in #21). **Fix path (when needed):** wall-clock the post-answer advance too — track `answerLockedAt = Date.now()` on answer registration; on AppState `active`, check if ≥1000ms have passed and force-advance if so. ~30 minutes of work plus testing. **Defer rationale:** single anecdotal occurrence in session 26, never reproduced reliably since, self-resolves in ~1 second, well-understood fix path documented above. Doing it now is polish-before-substance — Phase 4 takes priority. **Revisit trigger:** multiple beta or TestFlight testers report stuck/grayed buttons mid-round, OR CD hits it again on device. **Permanent kill trigger:** if it's still parked at Phase 7 pre-launch polish with zero new reports, close it permanently as a known-edge-case non-issue.
...
#### Phase 4 hardening discipline (security + reliability)

*Added 2026-05-16 session 31 after a senior-engineer forum post on the four mistakes that show up in every vibecode-built app. These are not "ideas to consider" — they're hard gates on Phase 4 work. Every Edge Function PR, every schema migration, every secrets touch should be reviewable against these four items.*

54. **Secret management discipline.** *Identified 2026-05-16 session 31.* No secrets ever committed to the repo. Rules:
    - `.gitignore` excludes all `.env*` files with `.env.example` as the single committed template (widened 2026-05-16 from the Expo default `.env*.local`, which only caught local-suffixed files and would have tracked a plain `.env` by default).
    - Production secrets ship via **EAS Secrets** for Expo builds — never inline in `app.json`, never in code, never in CI environment files visible to forks.
    - Supabase `service_role` key is **server-side only** — Edge Functions or admin scripts. Never imported by any file under `app/` or `components/`. Anti-pattern: the convenience of "just use service_role for now" during dev. Use the anon key + RLS or it doesn't ship.
    - RevenueCat **secret API key** is server-side only (webhook handler Edge Function). The client uses only the public SDK key.
    - Pre-commit audit grep on every commit during Phase 4 onward: `git diff --cached | grep -iE "(KEY|SECRET|TOKEN|PASSWORD).*=.*['\"][a-z0-9_-]{20,}" | grep -v ".example"` — must return empty. Worth wiring as a husky/lefthook hook once we add one.
    - Acceptance for Phase 4 close: a public repo audit (manual or automated) confirms zero real secrets in tree or in history. If anything leaks, rotate the affected key immediately, then git-filter-repo.
    - **Today's state (2026-05-16):** clean. No `.env` in tree, no `.env` in history, no secret-shaped strings in tracked files. The `.gitignore` widening is preventive against future drift.

55. **RLS on every Phase 4 table — default-deny, explicit allow.** *Identified 2026-05-16 session 31.* Schema discipline:
    - Every table created in `supabase/phase4_schema.sql` (and any future migration) gets `ALTER TABLE <name> ENABLE ROW LEVEL SECURITY;` in the same migration that creates it. No table ships without RLS, even tables that look "obviously read-only" (e.g., `ranks`, `emotes`) — those tables get an explicit `SELECT` policy granting public read instead.
    - Default rule for player-owned data: `auth.uid() = player_id` for SELECT, INSERT, UPDATE, DELETE on own rows. Cross-player reads (leaderboards, league standings, ghost pool) get explicit narrow SELECT policies that expose only the necessary columns (no email, no purchase history, no internal skill tier).
    - **Anti-pattern guard:** no use of `service_role` from client code as an RLS workaround. If a query "won't work with RLS," the policy is wrong, not the architecture.
    - **Acceptance test for Phase 4 close:** spin up two test players A and B. Sign in as A. Attempt direct table reads against B's `players`, `sessions`, `ghost_pool`, `pro_entitlements`, `streak_shields`, `push_tokens`, `league_memberships` rows. All seven should fail at the PostgREST/RLS layer with 403 or empty result — not "the app filters them out client-side."
    - **Today's state (2026-05-16):** both `supabase/schema.sql` (Phase 3 content) and `supabase/phase4_schema.sql` (Phase 4 draft) have zero RLS statements. Captured here as the Phase 4 acceptance gate. Schema work cannot ship without RLS landed in the same migration.

56. **Rate limiting on every public Edge Function.** *Identified 2026-05-16 session 31.* Backend abuse / cost control:
    - Every Supabase Edge Function we ship gets application-layer rate limiting. Supabase's built-in rate limits are coarse (per-IP, per-anon-key) and not granular enough for per-player abuse patterns.
    - **Baseline:** 60 requests/minute per authenticated player UUID. Burst allowance to 120/minute for short windows (legitimate spam-tap patterns during gameplay). Anonymous-session players capped tighter — 30/minute — since the abuse vector is lowest-friction there.
    - **Score submission specifically:** server validates `session_id` is unique (one submission per round, deduped at DB level via unique constraint), `round_id` falls within an active 6am-ET window for that player's join. Hard-rejects duplicate or out-of-window submissions.
    - **Daily Race specifically:** server-side dedup on (`player_id`, `race_date`) — a player who somehow gets two submissions past the client lockout (clock manipulation, parallel sessions) sees the second rejected with HTTP 409. This is the server-side half of the Appendix D #6 anti-cheat work; the client-side lockout is convenience UX, not security.
    - **Implementation candidates:** Postgres-based rate limiter (a `rate_limits` table keyed on `(player_id, endpoint, window_start)` with periodic cleanup) is simplest; Upstash Redis is an upgrade path if Postgres write load becomes a concern.
    - **Acceptance test for Phase 4 close:** scripted load test hits each Edge Function at 10× the rate limit; the function must return 429 cleanly, log the abuse to a `rate_limit_violations` table for review, and never crash, time out, or run up unbounded compute.

57. **Error handling past the happy path — every external call wrapped.** *Identified 2026-05-16 session 31.* Reliability discipline that has to be baked in from the first Edge Function, not retrofitted:
    - Every external API call (Supabase from client, RevenueCat webhook, Apple receipt validation, PostHog event capture, Sentry error capture, any future third-party) is wrapped in `try/catch` with structured error logging. No silent failures.
    - Every catch block either: (a) retries with exponential backoff for transient errors (network timeout, 5xx), (b) surfaces a user-facing message with a clear next step, or (c) pings Sentry with enough context for postmortem. The default of "log to console and move on" is the failure mode this rule is preventing.
    - **Network timeouts:** 5s default for non-critical calls (PostHog event, analytics ping), 10s for critical calls (auth, payment validation, score submission). No call ever blocks the 60-second round loop — this is the "never block a 60s round on a network call" principle from Phase 4 already restated as a hard rule.
    - **Client-side graceful degradation:** if Supabase is unreachable, the app continues in offline mode (Quickmatch falls back to cached question sets and local-only score recording with a deferred-sync queue, per the Appendix D #3 offline spec). The player never sees a spinning loader that never resolves.
    - **Acceptance test for Phase 4 close:** every Edge Function has a "third-party failure path" integration test — what happens when RevenueCat times out, when Apple's receipt service returns 503, when Supabase RPC fails. All should resolve to a defined error path, not silent failure or unhandled rejection.
    - **Cross-reference:** the incident runbook (`incident_runbook.md`, drafted v1.38 session 22e) covers the operational half — what we do when these failures actually fire. This Appendix D item covers the code-discipline half — making sure the failure paths exist in the code in the first place.
...
*End of doc v1.53 — last updated 2026-05-16.*
