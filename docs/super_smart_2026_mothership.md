...
**Status:** v1.24 — Consistency audit: five stale references from the v1.23 bot-ghost reversal cleaned up (onboarding first-round spec, Phase 3 exit criteria mirror, decision-log cross-reference, Appendix D #22 resolution, Appendix D resolved-section wording). No spec changes. v1.23 locked the bot-ghost system + Phase 4 schema draft (`supabase/phase4_schema.sql`). v1.22 clarified skill vs league tier.
...
**Why this works at low player counts:** the transition window seeding and forming queue (see League formation rules above) mean every league opens with real, recently active tier peers. A league of 8 Contenders is a real league — nobody gets force-mixed into a foreign tier just to hit a count threshold. Growth is organic — as the player base grows, each tier's population grows and clusters get fuller without any structural change.
...
- **League tier** — one of 8 named brackets in the weekly league ladder (Newcomer → Regular → Contender → Veteran → Qualifier → Finalist → Champion → Legend). Visible on the avatar as a coloured border stroke. Determines which league cohort you're placed in next week. Separate from, and independent of, **skill tier** below.
...
*End of doc v1.24 — last updated 2026-04-24.*
