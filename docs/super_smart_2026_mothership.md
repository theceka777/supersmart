...
**Status:** v1.25 — Mini-decisions bundle: 5 small Appendix D items locked (league tiers 1–3 renamed with entry at tier 2, splash 2s every launch, emoji policy, question retirement three-tier system with "Contact the developer" email in Profile in place of in-app report button; #40 support domain deferred). League ladder now Rookie → Newcomer → Regular → Veteran → Qualifier → Finalist → Champion → Legend. v1.24 cleaned up bot-ghost reversal references; v1.23 locked the bot-ghost system + Phase 4 schema draft.
