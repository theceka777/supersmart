...
**Status:** v1.26 — Phase 1 audit kicked off. First 100 questions of `1001.xml` (all `science` category) tagged against the Keep / Light edit / Heavy edit / Retire rubric through two full creative-director review passes. **Final locked split: 81 Keep / 19 Light edit / 0 Heavy edit / 0 Retire.** Deliverables in `audit_1001/` at the parent folder (mirrored into `supersmart/docs/audit_1001/`): tags CSV, methodology doc with 13 edge-case rulings, a review-before-commit protocol, and a "How to run the next batch" playbook that future sessions follow for Q101+. Three new methodology rulings locked during this session's reviews: trademark-evocative misspellings are features not bugs (Q19 `jumangy` preserves Jumanji evocation without trademark infringement); hemisphere disambiguation supports two acceptable patterns ("Northern Hemisphere winter solstice" + "the June solstice" both valid); tautology is acceptable when the creative director says it's acceptable. Science is the most durable category in the bank — do not project 81/19/0/0 onto the full 1001; the corpus-wide 700/150/100/50 projection from Part 8 remains the working estimate, with misc/music/movies/people carrying most of the Heavy+Retire load when those batches land. v1.25 bundled 5 mini-decisions (league tiers 1–3 renamed with entry at tier 2, splash 2s every launch, emoji policy, question retirement three-tier system with "Contact the developer" email in Profile in place of in-app report button; #40 support domain deferred). League ladder now Rookie → Newcomer → Regular → Veteran → Qualifier → Finalist → Champion → Legend.
...
**Free tier:** All 2,500 launch questions, both game modes, all multiplayer (Echo, Challenge, Leaderboards), all avatar customization. 5–7 free rounds per day included. No ads, anywhere, ever. *Note: the free bank is static at launch — seasonal packs (post-launch) are Pro-only. See decision log 2026-04-24.*
...
- ⏳ Audit all 1001 questions from the XML: Keep / Light edit / Heavy edit / Retire. **First 100 tagged 2026-04-24 (session 13)** — methodology doc + append-ready CSV live in `audit_1001/` at the parent folder; split 79/21/0/0 on this first batch (100% science). 901 questions to go — batches of ~100 per focused sitting expected.
